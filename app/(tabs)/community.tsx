    ...
    <SafeAreaView className="flex-1 bg-semantic-background-base">
      ...
      <ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 120 }}>
        ...
        {partners.map((partner, index) => (
          <GlassCard key={partner.id} delay={index * 100} style={{ marginBottom: 16 }}>
             <View className="flex-row items-center justify-between">
               <View className="flex-row items-center">
                 <View className="w-12 h-12 bg-emerald-100 rounded-full items-center justify-center mr-4">
                   <Typography variant="h3" color="primary">{partner.name.charAt(0)}</Typography>
                 </View>
                 <View>
        ...
        {/* Nearby Masjids */}
        <Typography variant="h3" style={{ marginTop: 24, marginBottom: 16 }}>Nearby Masjids</Typography>

        {loadingMasjids ? (
           <ActivityIndicator size="small" color="#0F6D5B" />
        ) : (
          masjids.map((masjid, index) => (
            <GlassCard key={masjid.id} delay={200 + index * 100} style={{ marginBottom: 16 }}>
              <View className="flex-row items-start justify-between">
                <View className="flex-1 pr-4">
                  ...
                  <Typography variant="body-sm" color="primary" style={{ marginTop: 8 }}>
